...
> _Dokumen ini **di simpan** sebagai `SOP.md`._  
...
2. **Loyang latihan tiap tugas**; setiap pekerjaan kecil—memperbaiki typo, menambah fitur, meng-upgrade paket—dibuatkan **cabang tugas** dari `preview`. Di sini kita bebas salah adonan, asal di akhirnya dibuatkan **cabang audit-RC** untuk menguji semua adonan menjadi roti terbaik.

3. **Kotak hampers** itu adalah **release-branch** yang berisi roti pesanan tiap klien yang punya **lisensi berstempel** (tag `vX.Y.Z-codename`) yang siap diantar—lengkap dengan **kartu ucapan** (Release Notes).
...
>   - _**release-branch (per lisensi)**: `release/vX.Y.Z-codename` → **di-handover** sebagai repo terpisah._
> - _Peran dokumen (tetap):_
>   - _`SOP.md`: dokumen ini._
>   - _`README.md`: muka depan—badge dinamis, link rilis terbaru, info singkat._
>   - _`CHANGELOG.md`: format **Keep a Changelog** sederhana: bagian `## [Unreleased]` diisi setiap task; saat rilis, dipindahkan ke `## [X.Y.Z] – codename`._
...
### 1.1 Memulai Dengan Sinkronisasi Diawal
...
```bash
yarn version --new-version X.Y.Z-dev.N
# Naikkan N per tugas.
# Contoh:
# yarn version --new-version 2.6.0-dev.1
```
...
```bash
git add -A
# Jika setelah komit lupa/belum bump versi dev, atau menambahkan changelog,
# silahkan bump versi dev, atau update 'CHANGELOG.md' lalu lakukan ini:
...
2. Review global, stabilisasi minor/minor bug fix jika perlu (tetap kecil).
3. Cek `CHANGELOG.md` — pastikan semua poin sudah di `## [Unreleased]`.
4. Naikkan **kandidat rilis** (bump versi RC di **cabang audit-RC**):

   ```bash
   yarn version --new-version X.Y.Z-rc.N
   # Naikkan N per audit-RC.
   # Contoh:
   # yarn version --new-version 2.6.0-rc.0
...
8. Staging (dari `preview`):
   ...
   - Pastikan selalu menaikan RC: `-rc.1`, `-rc.2`, ..., kalau mengulang langkah di atas.
...
> _Setelah huruf `v` menggunakan titik, `v.X.Y.Z` itu kostum tidak lazim di festival ini._
...
- Bersihkan versi dari suffix:
  ...
  # Contoh:
  # yarn version --new-version 2.6.0
  ```
...
- Lalu komit dan dorong **release-branch**:
...
- Buat **GitHub Release** untuk repo `pabrikroti-<kodenama>`, lampirkan:
  ...
  - Tulis judul dengan format `Release X.Y.Z (<kodenama>)`.
  - Lampirkan `CHANGELOG.md` bagian `## [X.Y.Z – <kodenama>]` menggantikan isi bagian `### 0.1.0 🧭 This Release Notes`.
  - Hapus semua is catatan terdahulu.
...
yarn version --new-version X.Y.Z-dev.0
# Contoh:
